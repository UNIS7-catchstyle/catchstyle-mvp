            ...
            {(rankings.length > 0 ? rankings : popularNames.map((name, index) => ({ rank: index + 1, keyword: name }))).map((item, index) => {
              ...
              return (
                <li key={index}>
                  ...
                  <span className={`trend trend-${trendDirection}`} aria-label={trendDirection === 'neutral' ? 'same' : trendDirection}>
                    {trendDirection === 'neutral' ? (
                      <span className="trend-neutral-mark" aria-hidden="true">-</span>
                    ) : (
